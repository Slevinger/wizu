...
class APIToDoListDb {
  ...
  async changeListNameAndDescription(list_id, new_name, new_descr, respond) {
    try {
      if (!list_id || (!new_name && new_descr) || (new_name && !new_descr)) {
        respond(
          401,
          " changeListName(list_id, new_name,respond)::no list_id/new_name provided"
        );
      } else {
        let resObj = {};
        ...
        let res = await this.dbo
          ...
          .updateOne({ _id: new ObjectId(list_id) }, { $set: resObj });
        if (res.result.n == 0) {
          respond(
            400,
            `changeListNameAndDescription(list_id, new_name, new_descr, respond):: could not find any lists with the id ${list_id}`
          );
        }
      }
    } catch (err) {
  ...
  async removeItemFromList(list_id, aid, respond) {
    try {
      if (!list_id || !new_name) {
        respond(
          401,
          " removeListFromEvent(list_id, event_id,respond)::no list_id/event_id provided"
        );
      } else {
        this.dbo
          .collection("events")
  ...
  async createItemInList(
    ...
    try {
      let credentials = await this.dataBase.validateToken(username, token);
      if (credentials) {
        let actionItemId = await this._createNewActionItem(
          username,
          descr,
          have,
          need,
  ...
  async addItemToList(list_id, aid, respond) {
    try {
      if (!list_id || !aid) {
        respond(
          401,
          " removeListFromEvent(list_id, aid,respond)::no list_id/aid provided"
        );
      } else {
        const res = await this.dbo
          .collection("todo_list")
  ...
  async addRowToActionItem(aid, username, quantity, payed, token, respond) {
    try {
      if (this.validateToken(token)) {
        const aItem = JSON.parse(JSON.stringify(meta().action_item));
        const aItemToAddTwo = await dbo
  ...
  async addListToEvent(list_id, event_id, respond) {
    try {
      if (event_id && list_id) {
        ...
        respond(
          400,
          "addListToEvent(list_id,event_id,respond)::your list_id or event_id is null"
        );
      }
    } catch (err) {
      respond(401, err);
  ...
  async _createNewActionItem(
    ...
    const aItem = JSON.parse(JSON.stringify(meta().action_item));
    let nextActionItemId = await this.dbo
      .collection("action_items")
      .find({}, { _id: 0, aid: 1 })
      .sort({ aid: -1 })
      .limit(1)
    ...
    aItem.unit = unit;
    aItem.cost = cost;
    if (event_id) aItem.event_id = new ObjectId(eventId);
